...
返回*this的成员函数
    ...
    基于const的重载
		...
		和之前的一样,当一个成员调用另外一个成员时,this指针在其中隐式地传递.
		因此当display调用do_display时,它的this指针隐式地传递给 do_display,而当display的非常量版本调用do_display时,
		它的this指针将隐式地从指向非常量指针转换位指向常量的指针

		建议:对于公共代码使用私有功能函数
		为什么要费力定义一个单独的do_display函数.
		一个基本愿望是避免在多处使用相同的代码
		预期随着类的规模发展,display函数有可能变得更加复杂,此时把相应的操作写在一处的作用就比较明显了
		我们很可能在开发过程中给do_display函数添加某些调试,在do_display一处添加或删除更容易		
类类型
	每个类定义了唯一的类型.对于两个类来说,即使它们的成员完全一样,这两个类也是不同类型.
	类的声明			
		就像可以把函数的声明和定义分离开来一样,我们也能仅仅声明类而暂时不定义它:		class Screen;		//类的声明
		前向声明, Screen在声明后定义前是一个不完全类型,只知道它是一个类类型,但不清楚它到底包含哪些成员       
		
		对一个类,在我们创建它的对象之前该类必须被定义过,而不能仅仅被声明.否则,编译器就无法了解这样的对象需要多少存储空间.
		类只能先被定义,才能用引用或者指针访问其成员.
		
		直到类被定义之后数据成员才能被声明成这种类类型.换句话说,我们必须首先完成类的定义,然后编译器才能知道存储该数据成员需要多少空间.
		因为只有当类全部完成后类才算被定义,所以一个类的成员类型不能是该类自己.然而,一旦一个类的名字出现后,他就被认为是声明过了,
...
友元再谈
	类之间的友元关系
		我们的Window_mgr类的某些成员可能需要访问它管理的Screen类的内部数据.例如,假设我们需要为Window_mgr添加一个名为clear的成员,
		它负责把一个指定的Screen的内容都设为空白.为了完成这一任务,clear需要访问Screen的私有成员;
		而要想令这种访问合法,Screen需要把WIndow_mgr指定成它的友元:
			class Screen
			{
				 //Windouw_mgr 的成员可以访问Screen类的私有部分
				 friend class Window_mgr;		
				...
		如果一个类指定了友元类,则友元类的成员函数可以访问此类包括非公有成员在内的所以成员.
		通过上面声明,Window_mgr被指定为Screen的友元,因此我们可以将Window_mgr的clear成员写成如下的形式:
		
		一开始,先把s定义成screens vector中第i个位置桑的Screen的引用,随后利用Screen的height和width成员计算出一个新的string对象,
		并令其含有若干个空白字符,最后我们把这个含有很多空白字符串赋给contents成员

		每个类负责控制自己的友元类或友元函数
	令成员函数作为友元
		除了令整个Window_mgr作为友元之外,Screen还可以只为clear 提供访问权限.当把一个成员函数声明成友元时,必须明确指出该成员函数属于哪个类
		class Screen{
